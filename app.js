...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Quy ra danh sách tab được phép theo (vai trò, phòng ban). Lạ → tối thiểu, KHÔNG bao giờ rơi về toàn quyền.
  function resolveAllowedTabs(role, dept) {
    if (role === "CEO") return GD_ALL_TABS;          // CEO: toàn quyền
  ...
  function tabsLabelFor(role, dept) {
    if (role === "CEO") return "Toàn quyền (12 tab)";
    return resolveAllowedTabs(role, dept).map(function (t) { return TAB_LABELS[t] || t; }).join(", ");
  }
  ...
  function applyRoleUiAccess(role, dept) {
    const allowedTabs = resolveAllowedTabs(role, dept);
    const navItems = document.querySelectorAll(".nav-item");
    ...
    try {
      ...
      if (saved) {
        ...
        accounts.forEach(acc => {
          ...
          const want = tabsLabelFor(acc.role, acc.dept);
  ...
  function initLoginSystem() {
    ...
    function verifyUserSessionAndConfigureUi() {
      ...
      applyRoleUiAccess(user.role, user.dept);
  ...
  function saveUserEditChanges() {
    ...
    addAuditLogEntry(currentPersona, `Cập nhật thông tin tài khoản ${accounts[index].name}`, `Email mới: ${newEmail}`);
    window.closeUserEditModal();
    renderRbacUserTable();
    showToast(`Đã cập nhật thông tin tài khoản ${accounts[index].name} thành công!`, "success");
  }
